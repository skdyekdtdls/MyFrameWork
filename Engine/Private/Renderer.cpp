...
HRESULT CRenderer::Initialize_Prototype()
{
	...
	if (FAILED(m_pTarget_Manager->Add_RenderTarget(m_pDevice, m_pContext,
		TEXT("Target_PreBackBuffer"), ViewPortDesc.Width, ViewPortDesc.Height, DXGI_FORMAT_B8G8R8A8_UNORM, _float4(0.f, 0.f, 0.f, 0.f))))
		return E_FAIL;
	...
	if (FAILED(m_pTarget_Manager->Add_MRT(TEXT("MRT_Pre"), TEXT("Target_PreBackBuffer"))))
		return E_FAIL;
	...
	m_pPostShader = CShader::Create(m_pDevice, m_pContext, TEXT("../Bin/ShaderFiles/Shader_PostProcessing.hlsl")
		, VTXPOSTEX_DECL::Elements, VTXPOSTEX_DECL::iNumElements);
	NULL_CHECK_RETURN(m_pPostShader, E_FAIL);

	m_pVIBuffer = CVIBuffer_Rect::Create(m_pDevice, m_pContext);
	NULL_CHECK_RETURN(m_pVIBuffer, E_FAIL);

	m_pPostVIBuffer = CVIBuffer_Rect::Create(m_pDevice, m_pContext);
	NULL_CHECK_RETURN(m_pPostVIBuffer, E_FAIL);
	...
	m_pTarget_Manager->Ready_Debug(TEXT("Target_PreBackBuffer"), 300.f, 500.f, 200.f, 200.f);
#endif // _DEBUG
	return S_OK;
}
...
{
	//m_pTarget_Manager->Begin_PreMRT(m_pContext, L"MRT_Pre");
	...
	//m_pTarget_Manager->End_PreMRT(m_pContext);
...
HRESULT CRenderer::Render_Lights()
{
	...
	/* Shade(0) + Specular(1) */
	if (FAILED(m_pTarget_Manager->Begin_PreMRT(m_pContext, TEXT("MRT_Lights"))))
		return E_FAIL;
	...
	if (FAILED(m_pTarget_Manager->End_PreMRT(m_pContext)))
		return E_FAIL;
...
{
	FAILED_CHECK_RETURN(m_pTarget_Manager->Bind_ShaderResourceView(TEXT("Target_PreBackBuffer")
		, m_pPostShader, "g_PreBackBufferTexture"), E_FAIL);

	FAILED_CHECK_RETURN(m_pPostShader->Bind_Matrix("g_WorldMatrix", &m_WorldMatrix), E_FAIL);
	FAILED_CHECK_RETURN(m_pPostShader->Bind_Matrix("g_ViewMatrix", &m_ViewMatrix), E_FAIL);
	FAILED_CHECK_RETURN(m_pPostShader->Bind_Matrix("g_ProjMatrix", &m_ProjMatrix), E_FAIL);
...
void CRenderer::Free(void)
{
	...
	Safe_Release(m_pPostVIBuffer);
	Safe_Release(m_pPostShader);
}
