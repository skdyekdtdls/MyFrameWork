...
_uint SM_TrozoPared::SM_TrozoPared_Id = 0;

SM_TrozoPared::SM_TrozoPared(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

SM_TrozoPared::SM_TrozoPared(const SM_TrozoPared& rhs)
	: CGameObject(rhs)
{
}

HRESULT SM_TrozoPared::Initialize_Prototype()
{
...
HRESULT SM_TrozoPared::Initialize(void* pArg)
{
	...
	++SM_TrozoPared_Id;
	m_tInfo.wstrName = TO_WSTR("SM_TrozoPared" + to_string(SM_TrozoPared_Id));
	m_tInfo.wstrKey = ProtoTag();
	m_tInfo.ID = SM_TrozoPared_Id;

	tagSM_TrozoParedDesc tCloneDesc;
	if (nullptr != pArg)
		tCloneDesc = *(tagSM_TrozoParedDesc*)pArg;
...
void SM_TrozoPared::Tick(_double TimeDelta)
{
...
void SM_TrozoPared::Late_Tick(_double TimeDelta)
{
...
void SM_TrozoPared::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void SM_TrozoPared::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
	m_pTransformCom->Load(hFile, dwByte, iLevelIndex);
}

HRESULT SM_TrozoPared::Add_Components()
{
	...
	FAILED_CHECK_RETURN(__super::Add_Component(eLevelID, L"Prototype_Component_Model_SM_TrozoPared", L"Com_Model", (CComponent**)&m_pModelCom, &tModelDesc), E_FAIL);
...
HRESULT SM_TrozoPared::SetUp_ShaderResources()
{
...
SM_TrozoPared* SM_TrozoPared::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	SM_TrozoPared* pInstance = new SM_TrozoPared(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created SM_TrozoPared");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* SM_TrozoPared::Clone(void* pArg)
{
	SM_TrozoPared* pInstance = new SM_TrozoPared(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned SM_TrozoPared");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void SM_TrozoPared::Free(void)
{
	__super::Free();
