...
_uint BushA::BushA_Id = 0;

BushA::BushA(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

BushA::BushA(const BushA& rhs)
	: CGameObject(rhs)
{
}

HRESULT BushA::Initialize_Prototype()
{
...
HRESULT BushA::Initialize(void* pArg)
{
	...
	++BushA_Id;
	m_tInfo.wstrName = TO_WSTR("BushA" + to_string(BushA_Id));
	m_tInfo.wstrKey = ProtoTag();
	m_tInfo.ID = BushA_Id;

	tagBushADesc tCloneDesc;
	if (nullptr != pArg)
		tCloneDesc = *(tagBushADesc*)pArg;
...
void BushA::Tick(_double TimeDelta)
{
...
void BushA::Late_Tick(_double TimeDelta)
{
...
void BushA::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void BushA::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
	m_pTransformCom->Load(hFile, dwByte, iLevelIndex);
}

HRESULT BushA::Add_Components()
{
	...
	FAILED_CHECK_RETURN(__super::Add_Component(eLevelID, L"Prototype_Component_Model_BushA", L"Com_Model", (CComponent**)&m_pModelCom, &tModelDesc), E_FAIL);
...
HRESULT BushA::SetUp_ShaderResources()
{
...
BushA* BushA::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	BushA* pInstance = new BushA(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created BushA");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* BushA::Clone(void* pArg)
{
	BushA* pInstance = new BushA(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned BushA");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void BushA::Free(void)
{
	__super::Free();
