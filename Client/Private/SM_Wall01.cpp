...
_uint SM_Wall01::SM_Wall01_Id = 0;

SM_Wall01::SM_Wall01(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

SM_Wall01::SM_Wall01(const SM_Wall01& rhs)
	: CGameObject(rhs)
{
}

HRESULT SM_Wall01::Initialize_Prototype()
{
...
HRESULT SM_Wall01::Initialize(void* pArg)
{
	...
	++SM_Wall01_Id;
	m_tInfo.wstrName = TO_WSTR("SM_Wall01" + to_string(SM_Wall01_Id));
	m_tInfo.wstrKey = ProtoTag();
	m_tInfo.ID = SM_Wall01_Id;

	tagSM_Wall01Desc tCloneDesc;
	if (nullptr != pArg)
		tCloneDesc = *(tagSM_Wall01Desc*)pArg;
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMLoadFloat4(&tCloneDesc.vPosition));

#ifdef _DEBUG
	__super::SetPickRadius(4.f * 100.f * m_pModelCom->GetPivotMatrixScale().x);
#endif

	return S_OK;
}

void SM_Wall01::Tick(_double TimeDelta)
{
...
void SM_Wall01::Late_Tick(_double TimeDelta)
{
...
void SM_Wall01::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void SM_Wall01::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
	m_pTransformCom->Load(hFile, dwByte, iLevelIndex);
}

HRESULT SM_Wall01::Add_Components()
{
	...
	FAILED_CHECK_RETURN(__super::Add_Component(eLevelID, L"Prototype_Component_Model_SM_Wall01", L"Com_Model", (CComponent**)&m_pModelCom, &tModelDesc), E_FAIL);
...
HRESULT SM_Wall01::SetUp_ShaderResources()
{
...
SM_Wall01* SM_Wall01::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	SM_Wall01* pInstance = new SM_Wall01(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created SM_Wall01");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* SM_Wall01::Clone(void* pArg)
{
	SM_Wall01* pInstance = new SM_Wall01(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned SM_Wall01");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void SM_Wall01::Free(void)
{
	__super::Free();
