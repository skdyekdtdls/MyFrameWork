...
void BatPotato_RIG::Tick(_double TimeDelta)
{
	...
	{
		m_pStateContextCom->TransitionTo(L"BatPotato_RIGDead");
		return;
	}
...
void BatPotato_RIG::Late_Tick(_double TimeDelta)
{
	...
	if (Facade->isRender(m_pRendererCom, m_pTransformCom))
	{
		...
		// Dead상태가 아니면 콜라이더 넣음
		if (0 != lstrcmp(L"BatPotato_RIGDead", m_pStateContextCom->GetCurState()))
...
void BatPotato_RIG::OnCollision(CCollider::COLLISION_INFO tCollisionInfo, _double TimeDelta)
{
	...
	{
		m_pStateContextCom->TransitionTo(TEXT("BatPotato_RIG_prawnHit"));
	}
...
void BatPotato_RIG::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void BatPotato_RIG::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
