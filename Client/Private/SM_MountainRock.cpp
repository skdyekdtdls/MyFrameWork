...
_uint SM_MountainRock::SM_MountainRock_Id = 0;

SM_MountainRock::SM_MountainRock(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

SM_MountainRock::SM_MountainRock(const SM_MountainRock& rhs)
	: CGameObject(rhs)
{
}

HRESULT SM_MountainRock::Initialize_Prototype()
{
...
HRESULT SM_MountainRock::Initialize(void* pArg)
{
	...
	++SM_MountainRock_Id;
	m_tInfo.wstrName = TO_WSTR("SM_MountainRock" + to_string(SM_MountainRock_Id));
	m_tInfo.wstrKey = ProtoTag();
	m_tInfo.ID = SM_MountainRock_Id;

	tagSM_MountainRockDesc tCloneDesc;
	if (nullptr != pArg)
		tCloneDesc = *(tagSM_MountainRockDesc*)pArg;
...
void SM_MountainRock::Tick(_double TimeDelta)
{
...
void SM_MountainRock::Late_Tick(_double TimeDelta)
{
...
HRESULT SM_MountainRock::Render()
{
...
void SM_MountainRock::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void SM_MountainRock::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
	m_pTransformCom->Load(hFile, dwByte, iLevelIndex);
}

HRESULT SM_MountainRock::Add_Components()
{
	...
	FAILED_CHECK_RETURN(__super::Add_Component(eLevelID, L"Prototype_Component_Model_SM_MountainRock", L"Com_Model", (CComponent**)&m_pModelCom, &tModelDesc), E_FAIL);
...
HRESULT SM_MountainRock::SetUp_ShaderResources()
{
...
SM_MountainRock* SM_MountainRock::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	SM_MountainRock* pInstance = new SM_MountainRock(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created SM_MountainRock");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* SM_MountainRock::Clone(void* pArg)
{
	SM_MountainRock* pInstance = new SM_MountainRock(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned SM_MountainRock");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void SM_MountainRock::Free(void)
{
	__super::Free();
