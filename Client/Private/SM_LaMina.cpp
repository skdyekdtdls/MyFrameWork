...
_uint SM_LaMina::SM_LaMina_Id = 0;

SM_LaMina::SM_LaMina(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

SM_LaMina::SM_LaMina(const SM_LaMina& rhs)
	: CGameObject(rhs)
{
}

HRESULT SM_LaMina::Initialize_Prototype()
{
...
HRESULT SM_LaMina::Initialize(void* pArg)
{
	...
	++SM_LaMina_Id;
	m_tInfo.wstrName = TO_WSTR("SM_LaMina" + to_string(SM_LaMina_Id));
	m_tInfo.wstrKey = ProtoTag();
	m_tInfo.ID = SM_LaMina_Id;

	tagSM_LaMinaDesc tCloneDesc;
	if (nullptr != pArg)
		tCloneDesc = *(tagSM_LaMinaDesc*)pArg;
...
void SM_LaMina::Tick(_double TimeDelta)
{
...
void SM_LaMina::Late_Tick(_double TimeDelta)
{
...
void SM_LaMina::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void SM_LaMina::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
	m_pTransformCom->Load(hFile, dwByte, iLevelIndex);
}

HRESULT SM_LaMina::Add_Components()
{
	...
	FAILED_CHECK_RETURN(__super::Add_Component(eLevelID, L"Prototype_Component_Model_SM_LaMina", L"Com_Model", (CComponent**)&m_pModelCom, &tModelDesc), E_FAIL);
...
HRESULT SM_LaMina::SetUp_ShaderResources()
{
...
SM_LaMina* SM_LaMina::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	SM_LaMina* pInstance = new SM_LaMina(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created SM_LaMina");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* SM_LaMina::Clone(void* pArg)
{
	SM_LaMina* pInstance = new SM_LaMina(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned SM_LaMina");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void SM_LaMina::Free(void)
{
	__super::Free();
