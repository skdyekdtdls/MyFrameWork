...
_uint MineCrystalsB::MineCrystalsB_Id = 0;

MineCrystalsB::MineCrystalsB(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

MineCrystalsB::MineCrystalsB(const MineCrystalsB& rhs)
	: CGameObject(rhs)
{
}

HRESULT MineCrystalsB::Initialize_Prototype()
{
...
HRESULT MineCrystalsB::Initialize(void* pArg)
{
	...
	++MineCrystalsB_Id;
	m_tInfo.wstrName = TO_WSTR("MineCrystalsB" + to_string(MineCrystalsB_Id));
	m_tInfo.wstrKey = ProtoTag();
	m_tInfo.ID = MineCrystalsB_Id;

	tagMineCrystalsBDesc tCloneDesc;
	if (nullptr != pArg)
		tCloneDesc = *(tagMineCrystalsBDesc*)pArg;
...
void MineCrystalsB::Tick(_double TimeDelta)
{
...
void MineCrystalsB::Late_Tick(_double TimeDelta)
{
...
void MineCrystalsB::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void MineCrystalsB::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
	m_pTransformCom->Load(hFile, dwByte, iLevelIndex);
}

HRESULT MineCrystalsB::Add_Components()
{
	...
	FAILED_CHECK_RETURN(__super::Add_Component(eLevelID, L"Prototype_Component_Model_MineCrystalsB", L"Com_Model", (CComponent**)&m_pModelCom, &tModelDesc), E_FAIL);
...
HRESULT MineCrystalsB::SetUp_ShaderResources()
{
...
MineCrystalsB* MineCrystalsB::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	MineCrystalsB* pInstance = new MineCrystalsB(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created MineCrystalsB");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* MineCrystalsB::Clone(void* pArg)
{
	MineCrystalsB* pInstance = new MineCrystalsB(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned MineCrystalsB");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void MineCrystalsB::Free(void)
{
	__super::Free();
