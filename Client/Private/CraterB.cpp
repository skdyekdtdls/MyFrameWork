...
_uint CraterB::CraterB_Id = 0;

CraterB::CraterB(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

CraterB::CraterB(const CraterB& rhs)
	: CGameObject(rhs)
{
}

HRESULT CraterB::Initialize_Prototype()
{
...
HRESULT CraterB::Initialize(void* pArg)
{
	...
	++CraterB_Id;
	m_tInfo.wstrName = TO_WSTR("CraterB" + to_string(CraterB_Id));
	m_tInfo.wstrKey = ProtoTag();
	m_tInfo.ID = CraterB_Id;

	tagCraterBDesc tCloneDesc;
	if (nullptr != pArg)
		tCloneDesc = *(tagCraterBDesc*)pArg;
...
void CraterB::Tick(_double TimeDelta)
{
...
void CraterB::Late_Tick(_double TimeDelta)
{
...
void CraterB::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void CraterB::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
	m_pTransformCom->Load(hFile, dwByte, iLevelIndex);
}

HRESULT CraterB::Add_Components()
{
	...
	FAILED_CHECK_RETURN(__super::Add_Component(eLevelID, L"Prototype_Component_Model_CraterB", L"Com_Model", (CComponent**)&m_pModelCom, &tModelDesc), E_FAIL);
...
HRESULT CraterB::SetUp_ShaderResources()
{
...
CraterB* CraterB::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CraterB* pInstance = new CraterB(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created CraterB");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CraterB::Clone(void* pArg)
{
	CraterB* pInstance = new CraterB(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned CraterB");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CraterB::Free(void)
{
	__super::Free();
