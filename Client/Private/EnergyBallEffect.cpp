...
_uint EnergyBallEffect::EnergyBallEffect_ID = 0;

EnergyBallEffect::EnergyBallEffect(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

EnergyBallEffect::EnergyBallEffect(const EnergyBallEffect& rhs)
	: CGameObject(rhs)
	, m_bEnable(false)
	, m_iCurrentIndex(-1)
{
...
HRESULT EnergyBallEffect::Initialize_Prototype()
{
...
HRESULT EnergyBallEffect::Initialize(void* pArg)
{
	...
	++EnergyBallEffect_ID;
	m_tInfo.wstrName = TO_WSTR("EnergyBallEffect" + to_string(EnergyBallEffect_ID));
...
void EnergyBallEffect::Tick(_double TimeDelta, _fvector vPos)
{
	if (false == m_bEnable)
		return;
...
void EnergyBallEffect::Late_Tick(_double TimeDelta)
{
	if (false == m_bEnable)
		return;
...
HRESULT EnergyBallEffect::Render()
{
...
void EnergyBallEffect::Reset()
{
	m_bEnable = true;
	m_iCurrentIndex = -1;
}

void EnergyBallEffect::Disable()
{
	m_bEnable = false;
}

HRESULT EnergyBallEffect::Add_Components()
{
...
HRESULT EnergyBallEffect::SetUp_ShaderResources()
{
...
EnergyBallEffect* EnergyBallEffect::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	EnergyBallEffect* pInstance = new EnergyBallEffect(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created EnergyBallEffect");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* EnergyBallEffect::Clone(void* pArg)
{
	EnergyBallEffect* pInstance = new EnergyBallEffect(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned EnergyBallEffect");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void EnergyBallEffect::Free(void)
{
	__super::Free();
