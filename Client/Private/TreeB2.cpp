...
_uint TreeB2::TreeB2_Id = 0;

TreeB2::TreeB2(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

TreeB2::TreeB2(const TreeB2& rhs)
	: CGameObject(rhs)
{
}

HRESULT TreeB2::Initialize_Prototype()
{
...
HRESULT TreeB2::Initialize(void* pArg)
{
	...
	++TreeB2_Id;
	m_tInfo.wstrName = TO_WSTR("TreeB2" + to_string(TreeB2_Id));
	m_tInfo.wstrKey = ProtoTag();
	m_tInfo.ID = TreeB2_Id;

	tagTreeB2Desc tCloneDesc;
	if (nullptr != pArg)
		tCloneDesc = *(tagTreeB2Desc*)pArg;
...
void TreeB2::Tick(_double TimeDelta)
{
...
void TreeB2::Late_Tick(_double TimeDelta)
{
...
void TreeB2::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void TreeB2::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
	m_pTransformCom->Load(hFile, dwByte, iLevelIndex);
}

HRESULT TreeB2::Add_Components()
{
	...
	FAILED_CHECK_RETURN(__super::Add_Component(eLevelID, L"Prototype_Component_Model_TreeB2", L"Com_Model", (CComponent**)&m_pModelCom, &tModelDesc), E_FAIL);
...
HRESULT TreeB2::SetUp_ShaderResources()
{
...
TreeB2* TreeB2::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	TreeB2* pInstance = new TreeB2(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created TreeB2");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* TreeB2::Clone(void* pArg)
{
	TreeB2* pInstance = new TreeB2(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned TreeB2");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void TreeB2::Free(void)
{
	__super::Free();
