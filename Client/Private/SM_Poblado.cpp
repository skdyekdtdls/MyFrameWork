...
_uint SM_Poblado::SM_Poblado_Id = 0;

SM_Poblado::SM_Poblado(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

SM_Poblado::SM_Poblado(const SM_Poblado& rhs)
	: CGameObject(rhs)
{
}

HRESULT SM_Poblado::Initialize_Prototype()
{
...
HRESULT SM_Poblado::Initialize(void* pArg)
{
	...
	++SM_Poblado_Id;
	m_tInfo.wstrName = TO_WSTR("SM_Poblado" + to_string(SM_Poblado_Id));
	m_tInfo.wstrKey = ProtoTag();
	m_tInfo.ID = SM_Poblado_Id;

	tagSM_PobladoDesc tCloneDesc;
	if (nullptr != pArg)
		tCloneDesc = *(tagSM_PobladoDesc*)pArg;
...
void SM_Poblado::Tick(_double TimeDelta)
{
...
void SM_Poblado::Late_Tick(_double TimeDelta)
{
...
void SM_Poblado::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void SM_Poblado::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
	m_pTransformCom->Load(hFile, dwByte, iLevelIndex);
}

HRESULT SM_Poblado::Add_Components()
{
	...
	FAILED_CHECK_RETURN(__super::Add_Component(eLevelID, L"Prototype_Component_Model_SM_Poblado", L"Com_Model", (CComponent**)&m_pModelCom, &tModelDesc), E_FAIL);
...
HRESULT SM_Poblado::SetUp_ShaderResources()
{
...
SM_Poblado* SM_Poblado::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	SM_Poblado* pInstance = new SM_Poblado(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created SM_Poblado");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* SM_Poblado::Clone(void* pArg)
{
	SM_Poblado* pInstance = new SM_Poblado(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned SM_Poblado");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void SM_Poblado::Free(void)
{
	__super::Free();
