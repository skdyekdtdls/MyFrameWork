...
_uint CraterC::CraterC_Id = 0;

CraterC::CraterC(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

CraterC::CraterC(const CraterC& rhs)
	: CGameObject(rhs)
{
}

HRESULT CraterC::Initialize_Prototype()
{
...
HRESULT CraterC::Initialize(void* pArg)
{
	...
	++CraterC_Id;
	m_tInfo.wstrName = TO_WSTR("CraterC" + to_string(CraterC_Id));
	m_tInfo.wstrKey = ProtoTag();
	m_tInfo.ID = CraterC_Id;

	tagCraterCDesc tCloneDesc;
	if (nullptr != pArg)
		tCloneDesc = *(tagCraterCDesc*)pArg;
...
void CraterC::Tick(_double TimeDelta)
{
...
void CraterC::Late_Tick(_double TimeDelta)
{
...
void CraterC::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void CraterC::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
	m_pTransformCom->Load(hFile, dwByte, iLevelIndex);
}

HRESULT CraterC::Add_Components()
{
	...
	FAILED_CHECK_RETURN(__super::Add_Component(eLevelID, L"Prototype_Component_Model_CraterC", L"Com_Model", (CComponent**)&m_pModelCom, &tModelDesc), E_FAIL);
...
HRESULT CraterC::SetUp_ShaderResources()
{
...
CraterC* CraterC::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CraterC* pInstance = new CraterC(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created CraterC");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CraterC::Clone(void* pArg)
{
	CraterC* pInstance = new CraterC(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned CraterC");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CraterC::Free(void)
{
	__super::Free();
