...
P1Attack04::P1Attack04(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: Bullet(pDevice, pContext)
{
}

P1Attack04::P1Attack04(const P1Attack04& rhs)
	: Bullet(rhs)
{
}

HRESULT P1Attack04::Initialize_Prototype()
{
...
HRESULT P1Attack04::Initialize(void* pArg)
{
	...
	++P1Attack04_Id;
	m_tInfo.wstrName = TO_WSTR("P1Attack04" + to_string(P1Attack04_Id));
	m_tInfo.wstrKey = ProtoTag();
	m_tInfo.ID = P1Attack04_Id;

	tagP1Attack04Desc tP1Attack04Desc;
	if (nullptr != pArg)
		tP1Attack04Desc = *(tagP1Attack04Desc*)pArg;
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMLoadFloat4(&tP1Attack04Desc.vPosition));
...
void P1Attack04::Tick(_double TimeDelta)
{
...
{
	m_bDead = false;
	_vector vPosition = XMLoadFloat4(&((tagP1Attack04Desc*)pArg)->vPosition);

	m_pTransformCom->Set_State(CTransform::STATE_POSITION, vPosition);
	m_pTimeCounterCom->Enable();
}

void P1Attack04::SetDead()
{
	__super::SetDead();
	ObjectPool<P1Attack04>::GetInstance()->PushPool(this);
	m_pTimeCounterCom->Reset();
	m_pTimeCounterCom->Disable();
}

HRESULT P1Attack04::Add_Components()
{
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
...
HRESULT P1Attack04::SetUp_ShaderResources()
{
...
void P1Attack04::OnCollision(CCollider::COLLISION_INFO tCollisionInfo, _double TimeDelta)
{
...
P1Attack04* P1Attack04::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	P1Attack04* pInstance = new P1Attack04(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created P1Attack04");
		Safe_Release(pInstance);
	}
	return pInstance;
}

Bullet* P1Attack04::Clone(void* pArg)
{
	P1Attack04* pInstance = new P1Attack04(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned P1Attack04");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void P1Attack04::Free(void)
{
	__super::Free();
