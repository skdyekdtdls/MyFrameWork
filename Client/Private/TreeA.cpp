...
_uint TreeA::TreeA_Id = 0;

TreeA::TreeA(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

TreeA::TreeA(const TreeA& rhs)
	: CGameObject(rhs)
{
}

HRESULT TreeA::Initialize_Prototype()
{
...
HRESULT TreeA::Initialize(void* pArg)
{
	...
	++TreeA_Id;
	m_tInfo.wstrName = TO_WSTR("TreeA" + to_string(TreeA_Id));
	m_tInfo.wstrKey = ProtoTag();
	m_tInfo.ID = TreeA_Id;

	tagTreeADesc tCloneDesc;
	if (nullptr != pArg)
		tCloneDesc = *(tagTreeADesc*)pArg;
...
void TreeA::Tick(_double TimeDelta)
{
...
void TreeA::Late_Tick(_double TimeDelta)
{
...
void TreeA::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void TreeA::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
	m_pTransformCom->Load(hFile, dwByte, iLevelIndex);
}

HRESULT TreeA::Add_Components()
{
	...
	FAILED_CHECK_RETURN(__super::Add_Component(eLevelID, L"Prototype_Component_Model_TreeA", L"Com_Model", (CComponent**)&m_pModelCom, &tModelDesc), E_FAIL);
...
HRESULT TreeA::SetUp_ShaderResources()
{
...
TreeA* TreeA::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	TreeA* pInstance = new TreeA(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created TreeA");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* TreeA::Clone(void* pArg)
{
	TreeA* pInstance = new TreeA(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned TreeA");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void TreeA::Free(void)
{
	__super::Free();
