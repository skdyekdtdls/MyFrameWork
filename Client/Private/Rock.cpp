...
_uint Rock::Rock_Id = 0;

/* Don't Forget Release for the VIBuffer or Model Component*/

Rock::Rock(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
...
HRESULT Rock::Initialize_Prototype()
{
...
HRESULT Rock::Initialize(void* pArg)
{
	...
	++Rock_Id;
	m_tInfo.wstrName = TO_WSTR("Rock" + to_string(Rock_Id));
...
void Rock::Tick(_double TimeDelta)
{
...
void Rock::Late_Tick(_double TimeDelta)
{
...
void Rock::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void Rock::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
	m_pTransformCom->Load(hFile, dwByte, iLevelIndex);
}

HRESULT Rock::Add_Components()
{
	...
	CModel::CMODEL_DESC tModelDesc; tModelDesc.pOwner = this;
	 FAILED_CHECK_RETURN(__super::Add_Component(eLevelID, L"Prototype_Component_Model_Rock", L"Com_Model", (CComponent**)&m_pModelCom, &tModelDesc), E_FAIL);
...
HRESULT Rock::SetUp_ShaderResources()
{
...
Rock* Rock::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	Rock* pInstance = new Rock(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created Rock");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* Rock::Clone(void* pArg)
{
	Rock* pInstance = new Rock(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned Rock");
		Safe_Release(pInstance);
	}
	return pInstance;
}
