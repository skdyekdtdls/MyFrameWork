...
_uint SM_MiniBujer::SM_MiniBujer_Id = 0;

SM_MiniBujer::SM_MiniBujer(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

SM_MiniBujer::SM_MiniBujer(const SM_MiniBujer& rhs)
	: CGameObject(rhs)
{
}

HRESULT SM_MiniBujer::Initialize_Prototype()
{
...
HRESULT SM_MiniBujer::Initialize(void* pArg)
{
	...
	++SM_MiniBujer_Id;
	m_tInfo.wstrName = TO_WSTR("SM_MiniBujer" + to_string(SM_MiniBujer_Id));
	m_tInfo.wstrKey = ProtoTag();
	m_tInfo.ID = SM_MiniBujer_Id;

	tagSM_MiniBujerDesc tCloneDesc;
	if (nullptr != pArg)
		tCloneDesc = *(tagSM_MiniBujerDesc*)pArg;
...
void SM_MiniBujer::Tick(_double TimeDelta)
{
...
void SM_MiniBujer::Late_Tick(_double TimeDelta)
{
...
void SM_MiniBujer::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void SM_MiniBujer::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
	m_pTransformCom->Load(hFile, dwByte, iLevelIndex);
}

HRESULT SM_MiniBujer::Add_Components()
{
	...
	FAILED_CHECK_RETURN(__super::Add_Component(eLevelID, L"Prototype_Component_Model_SM_MiniBujer", L"Com_Model", (CComponent**)&m_pModelCom, &tModelDesc), E_FAIL);
...
HRESULT SM_MiniBujer::SetUp_ShaderResources()
{
...
SM_MiniBujer* SM_MiniBujer::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	SM_MiniBujer* pInstance = new SM_MiniBujer(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created SM_MiniBujer");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* SM_MiniBujer::Clone(void* pArg)
{
	SM_MiniBujer* pInstance = new SM_MiniBujer(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned SM_MiniBujer");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void SM_MiniBujer::Free(void)
{
	__super::Free();
