...
_uint MineCrystalsA::MineCrystalsA_Id = 0;

MineCrystalsA::MineCrystalsA(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

MineCrystalsA::MineCrystalsA(const MineCrystalsA& rhs)
	: CGameObject(rhs)
{
}

HRESULT MineCrystalsA::Initialize_Prototype()
{
...
HRESULT MineCrystalsA::Initialize(void* pArg)
{
	...
	++MineCrystalsA_Id;
	m_tInfo.wstrName = TO_WSTR("MineCrystalsA" + to_string(MineCrystalsA_Id));
	m_tInfo.wstrKey = ProtoTag();
	m_tInfo.ID = MineCrystalsA_Id;

	tagMineCrystalsADesc tCloneDesc;
	if (nullptr != pArg)
		tCloneDesc = *(tagMineCrystalsADesc*)pArg;
...
void MineCrystalsA::Tick(_double TimeDelta)
{
...
void MineCrystalsA::Late_Tick(_double TimeDelta)
{
...
void MineCrystalsA::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void MineCrystalsA::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
	m_pTransformCom->Load(hFile, dwByte, iLevelIndex);
}

HRESULT MineCrystalsA::Add_Components()
{
	...
	FAILED_CHECK_RETURN(__super::Add_Component(eLevelID, L"Prototype_Component_Model_MineCrystalsA", L"Com_Model", (CComponent**)&m_pModelCom, &tModelDesc), E_FAIL);
...
HRESULT MineCrystalsA::SetUp_ShaderResources()
{
...
MineCrystalsA* MineCrystalsA::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	MineCrystalsA* pInstance = new MineCrystalsA(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created MineCrystalsA");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* MineCrystalsA::Clone(void* pArg)
{
	MineCrystalsA* pInstance = new MineCrystalsA(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned MineCrystalsA");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void MineCrystalsA::Free(void)
{
	__super::Free();
