...
_uint SM_Piedras::SM_Piedras_Id = 0;

SM_Piedras::SM_Piedras(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

SM_Piedras::SM_Piedras(const SM_Piedras& rhs)
	: CGameObject(rhs)
{
}

HRESULT SM_Piedras::Initialize_Prototype()
{
...
HRESULT SM_Piedras::Initialize(void* pArg)
{
	...
	++SM_Piedras_Id;
	m_tInfo.wstrName = TO_WSTR("SM_Piedras" + to_string(SM_Piedras_Id));
	m_tInfo.wstrKey = ProtoTag();
	m_tInfo.ID = SM_Piedras_Id;

	tagSM_PiedrasDesc tCloneDesc;
	if (nullptr != pArg)
		tCloneDesc = *(tagSM_PiedrasDesc*)pArg;
...
void SM_Piedras::Tick(_double TimeDelta)
{
...
void SM_Piedras::Late_Tick(_double TimeDelta)
{
...
void SM_Piedras::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void SM_Piedras::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
	m_pTransformCom->Load(hFile, dwByte, iLevelIndex);
}

HRESULT SM_Piedras::Add_Components()
{
	...
	FAILED_CHECK_RETURN(__super::Add_Component(eLevelID, L"Prototype_Component_Model_SM_Piedras", L"Com_Model", (CComponent**)&m_pModelCom, &tModelDesc), E_FAIL);
...
HRESULT SM_Piedras::SetUp_ShaderResources()
{
...
SM_Piedras* SM_Piedras::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	SM_Piedras* pInstance = new SM_Piedras(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created SM_Piedras");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* SM_Piedras::Clone(void* pArg)
{
	SM_Piedras* pInstance = new SM_Piedras(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned SM_Piedras");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void SM_Piedras::Free(void)
{
	__super::Free();
