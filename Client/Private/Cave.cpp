...
_uint Cave::Cave_Id = 0;

Cave::Cave(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
...
HRESULT Cave::Initialize_Prototype()
{
...
HRESULT Cave::Initialize(void* pArg)
{
	...
	++Cave_Id;
	m_tInfo.wstrName = TO_WSTR("Cave" + to_string(Cave_Id));
	m_tInfo.wstrKey = ProtoTag();
	m_tInfo.ID = Cave_Id;

	tagCaveDesc tCloneDesc;
	if (nullptr != pArg)
		tCloneDesc = *(tagCaveDesc*)pArg;
...
void Cave::Tick(_double TimeDelta)
{
...
void Cave::Late_Tick(_double TimeDelta)
{
...
void Cave::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void Cave::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
	m_pTransformCom->Load(hFile, dwByte, iLevelIndex);
}

HRESULT Cave::Add_Components()
{
	...
	FAILED_CHECK_RETURN(__super::Add_Component(eLevelID, L"Prototype_Component_Model_Cave", L"Com_Model", (CComponent**)&m_pModelCom, &tModelDesc), E_FAIL);
...
HRESULT Cave::SetUp_ShaderResources()
{
...
Cave* Cave::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	Cave* pInstance = new Cave(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created Cave");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* Cave::Clone(void* pArg)
{
	Cave* pInstance = new Cave(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned Cave");
		Safe_Release(pInstance);
	}
	return pInstance;
}
