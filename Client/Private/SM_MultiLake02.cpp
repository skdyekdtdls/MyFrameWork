...
_uint SM_MultiLake02::SM_MultiLake02_Id = 0;

SM_MultiLake02::SM_MultiLake02(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

SM_MultiLake02::SM_MultiLake02(const SM_MultiLake02& rhs)
	: CGameObject(rhs)
{
}

HRESULT SM_MultiLake02::Initialize_Prototype()
{
...
HRESULT SM_MultiLake02::Initialize(void* pArg)
{
	...
	++SM_MultiLake02_Id;
	m_tInfo.wstrName = TO_WSTR("SM_MultiLake02" + to_string(SM_MultiLake02_Id));
	m_tInfo.wstrKey = ProtoTag();
	m_tInfo.ID = SM_MultiLake02_Id;

	tagSM_MultiLake02Desc tCloneDesc;
	if (nullptr != pArg)
		tCloneDesc = *(tagSM_MultiLake02Desc*)pArg;
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMLoadFloat4(&tCloneDesc.vPosition));

#ifdef _DEBUG
	__super::SetPickRadius(7.f * 100.f * m_pModelCom->GetPivotMatrixScale().x);
#endif

	return S_OK;
}

void SM_MultiLake02::Tick(_double TimeDelta)
{
...
void SM_MultiLake02::Late_Tick(_double TimeDelta)
{
...
void SM_MultiLake02::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void SM_MultiLake02::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
	m_pTransformCom->Load(hFile, dwByte, iLevelIndex);
}

HRESULT SM_MultiLake02::Add_Components()
{
	...
	FAILED_CHECK_RETURN(__super::Add_Component(eLevelID, L"Prototype_Component_Model_SM_MultiLake02", L"Com_Model", (CComponent**)&m_pModelCom, &tModelDesc), E_FAIL);
...
HRESULT SM_MultiLake02::SetUp_ShaderResources()
{
...
SM_MultiLake02* SM_MultiLake02::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	SM_MultiLake02* pInstance = new SM_MultiLake02(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created SM_MultiLake02");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* SM_MultiLake02::Clone(void* pArg)
{
	SM_MultiLake02* pInstance = new SM_MultiLake02(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned SM_MultiLake02");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void SM_MultiLake02::Free(void)
{
	__super::Free();
