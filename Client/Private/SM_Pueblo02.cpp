...
_uint SM_Pueblo02::SM_Pueblo02_Id = 0;

SM_Pueblo02::SM_Pueblo02(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

SM_Pueblo02::SM_Pueblo02(const SM_Pueblo02& rhs)
	: CGameObject(rhs)
{
}

HRESULT SM_Pueblo02::Initialize_Prototype()
{
...
HRESULT SM_Pueblo02::Initialize(void* pArg)
{
	...
	++SM_Pueblo02_Id;
	m_tInfo.wstrName = TO_WSTR("SM_Pueblo02" + to_string(SM_Pueblo02_Id));
	m_tInfo.wstrKey = ProtoTag();
	m_tInfo.ID = SM_Pueblo02_Id;

	tagSM_Pueblo02Desc tCloneDesc;
	if (nullptr != pArg)
		tCloneDesc = *(tagSM_Pueblo02Desc*)pArg;
...
void SM_Pueblo02::Tick(_double TimeDelta)
{
...
void SM_Pueblo02::Late_Tick(_double TimeDelta)
{
...
void SM_Pueblo02::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void SM_Pueblo02::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
	m_pTransformCom->Load(hFile, dwByte, iLevelIndex);
}

HRESULT SM_Pueblo02::Add_Components()
{
	...
	FAILED_CHECK_RETURN(__super::Add_Component(eLevelID, L"Prototype_Component_Model_SM_Pueblo02", L"Com_Model", (CComponent**)&m_pModelCom, &tModelDesc), E_FAIL);
...
HRESULT SM_Pueblo02::SetUp_ShaderResources()
{
...
SM_Pueblo02* SM_Pueblo02::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	SM_Pueblo02* pInstance = new SM_Pueblo02(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created SM_Pueblo02");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* SM_Pueblo02::Clone(void* pArg)
{
	SM_Pueblo02* pInstance = new SM_Pueblo02(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned SM_Pueblo02");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void SM_Pueblo02::Free(void)
{
	__super::Free();
