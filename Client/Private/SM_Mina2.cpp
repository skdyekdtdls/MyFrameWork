...
_uint SM_Mina2::SM_Mina2_Id = 0;

SM_Mina2::SM_Mina2(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

SM_Mina2::SM_Mina2(const SM_Mina2& rhs)
	: CGameObject(rhs)
{
}

HRESULT SM_Mina2::Initialize_Prototype()
{
...
HRESULT SM_Mina2::Initialize(void* pArg)
{
	...
	++SM_Mina2_Id;
	m_tInfo.wstrName = TO_WSTR("SM_Mina2" + to_string(SM_Mina2_Id));
	m_tInfo.wstrKey = ProtoTag();
	m_tInfo.ID = SM_Mina2_Id;

	tagSM_Mina2Desc tCloneDesc;
	if (nullptr != pArg)
		tCloneDesc = *(tagSM_Mina2Desc*)pArg;
...
void SM_Mina2::Tick(_double TimeDelta)
{
...
void SM_Mina2::Late_Tick(_double TimeDelta)
{
...
void SM_Mina2::Save(HANDLE hFile, DWORD& dwByte)
{
	m_tInfo.Save(hFile, dwByte);
	m_pTransformCom->Save(hFile, dwByte);
}

void SM_Mina2::Load(HANDLE hFile, DWORD& dwByte, _uint iLevelIndex)
{
	m_pTransformCom->Load(hFile, dwByte, iLevelIndex);
}

HRESULT SM_Mina2::Add_Components()
{
	...
	FAILED_CHECK_RETURN(__super::Add_Component(eLevelID, L"Prototype_Component_Model_SM_Mina2", L"Com_Model", (CComponent**)&m_pModelCom, &tModelDesc), E_FAIL);
...
HRESULT SM_Mina2::SetUp_ShaderResources()
{
...
SM_Mina2* SM_Mina2::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	SM_Mina2* pInstance = new SM_Mina2(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created SM_Mina2");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* SM_Mina2::Clone(void* pArg)
{
	SM_Mina2* pInstance = new SM_Mina2(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned SM_Mina2");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void SM_Mina2::Free(void)
{
	__super::Free();
