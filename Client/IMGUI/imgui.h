...
struct ImGuiIO
{
    ...
    IMGUI_API void  AddKeyAnalogEvent(ImGuiKey key, bool down, float v);    // Queue a new key down/up event for analog values (e.g. ImGuiKey_Gamepad_ values). SetDead-zones should be handled by the backend.
...
//                 Tick ImGuiPlatformIO's Monitors list every frame.
//                 Tick MousePos every frame, in absolute coordinates.
// - Application:  In your main loop, call ImGui::UpdatePlatformWindows(), ImGui::RenderPlatformWindowsDefault() after EndFrame() or Render().
...
struct ImGuiPlatformIO
{
    ...
    // - Updated by: app/backend. Tick every frame to dynamically support changing monitor or DPI configuration.
